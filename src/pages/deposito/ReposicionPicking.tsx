...
export default function ReposicionPicking() {
  ...
  async function detectarBajos() {
    setGenerando(true)
    // Buscar ubicaciones picking con stock bajo mínimo
    const { data } = await supabase.rpc('get_ubicaciones_bajo_minimo').catch(() => ({ data: null }))
    if (!data) {
      // Fallback: buscar stock con cantidad <= cantidad_minima en ubicaciones tipo Picking
      ...
      setArticulosBajos(stockBajo?.filter((s: any) => s.cantidad <= (s.ubicaciones?.stock_minimo || 10)) || [])
    } else {
      setArticulosBajos(data)
    }
    setGenerando(false)
    setShowNuevo(true)
  }
  ...
  async function cambiarEstado(item: ItemReposicion, nuevoEstado: EstadoRepo) {
    ...
    if (nuevoEstado === 'completada') {
      // Mover stock de reserva a picking
      await supabase.rpc('ejecutar_reposicion', { reposicion_id: item.id }).catch(() => null)
    }
    await fetchItems()
    setProcesando(null)
  }
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: 'Completadas hoy', value: completadas, color: 'text-green-400' },
        ...
        <div className="flex gap-3 mb-4">
          ...
          <select value={filtroPrioridad} onChange={e => setFiltroPrioridad(e.target.value === '' ? '' : parseInt(e.target.value) as any)}
            className="input-dark rounded-lg px-3 py-2 text-sm">
              ...
              <tbody>
                {filtrados.length === 0 ? (
                  <tr><td colSpan={7} className="text-center py-10 text-dark-400">
                    No hay reposiciones {filtroEstado ? 'con ese estado' : ''}
                    {!filtroEstado && items.length === 0 && (
                      <p className="text-sm mt-2">Usá "Detectar Faltantes" para generar tareas automáticamente</p>
                    )}
                  </td></tr>
                ) : filtrados.map(item => (
          ...
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div className="card rounded-xl w-full max-w-md p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between"><span className="text-dark-400 text-sm">Descripción</span><span className="text-dark-300 text-sm text-right max-w-xs">{seleccionado.articulo_nombre}</span></div>
