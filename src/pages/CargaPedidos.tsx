...
// Cabecera opcional que puede venir desde el CSV
const COLS_CABECERA_OPT = [
  'tipopago',
  'iddocumento',
  'idempresa',
  'idsucur',
...
const MOTIVOS_VALIDOS: Record<string, string> = {
  '1': '[B',
  '2': 'CLIENTE ESPECIAL'
}
...
function normalizarStr(v: string | undefined) {
  return (v || '').trim()
}

function buildPedidoKey(row: Record<string, string>) {
  const npedido = normalizarStr(row[COL_NPEDIDO])
  const idcliente = normalizarStr(row.idcliente)
  const idclialias = normalizarStr(row.idclialias)
  const fecentre = normalizarStr(row.fecentre)
  const idempresa = normalizarStr(row.idempresa)
  const iddocumento = normalizarStr(row.iddocumento)

  // Importante: sumamos idempresa e iddocumento al agrupado
  // para evitar que se mezclen pedidos del mismo cliente/fecha
  // con distinta empresa o tipo de documento.
  return npedido
    ? `n${npedido}_${idcliente}_${idclialias}_${fecentre}_${idempresa}_${iddocumento}`
    : `${idcliente}_${idclialias}_${fecentre}_${idempresa}_${iddocumento}`
}
  ...
  for (const row of rows) {
    ...
    if (!mapa.has(key)) {
      ...
      mapa.set(key, {
        ...
        idclialias: row.idclialias,
        fecentre: row.fecentre,
        npedido: row[COL_NPEDIDO] || '',
        renglones: [],
        overrides,
        errores: []
      })
...
function validarPedidos(pedidos: PedidoAgrupado[]): PedidoAgrupado[] {
  return pedidos.map(p => {
    ...
    if (!p.idcliente || isNaN(Number(p.idcliente))) {
      errores.push('idcliente inválido')
    }

    if (!p.idclialias || isNaN(Number(p.idclialias))) {
      errores.push('idclialias inválido')
    }

    if (!p.fecentre || !/^\d{2}\/\d{2}\/\d{4}$/.test(p.fecentre)) {
      errores.push('fecentre debe ser DD/MM/YYYY')
    }

    if (p.overrides.idempresa !== undefined && p.overrides.idempresa !== '' && isNaN(Number(p.overrides.idempresa))) {
      errores.push('idempresa inválido')
    }

    if (p.overrides.iddocumento !== undefined && p.overrides.iddocumento.trim() === '') {
      errores.push('iddocumento inválido')
    }

    for (const r of p.renglones) {
      if (!r.codart || isNaN(Number(r.codart))) {
        errores.push(`Artículo "${r.codart}": codart inválido`)
      }

      if (!r.cant || isNaN(Number(r.cant))) {
        errores.push(`Artículo "${r.codart}": cant inválida`)
      }

      const tieneBonif = r.bonifpct && r.bonifpct !== '' && r.bonifpct !== '0'
      if (tieneBonif && (!r.motivo || !MOTIVOS_VALIDOS[r.motivo])) {
        errores.push(
          `Artículo "${r.codart}": bonifpct requiere motivo válido (1=${MOTIVOS_VALIDOS['1']}, 2=${MOTIVOS_VALIDOS['2']})`
        )
      }
    }
...
function descargarCSV(resultados: ResultadoPedido[]) {
  ...
  const rows = resultados.map(r =>
    `${r.idcliente};${r.fecentre};${r.nropedido};${r.estado};${r.error}`
  )
  ...
  a.href = url
  a.download = 'resultado_pedidos.csv'
  a.click()
...
async function procesarPedidoERP(
  ...
  const payload = {
    pedido: {
      idcliente: parseInt(pedido.idcliente, 10),
      idclialias: parseInt(pedido.idclialias, 10),
  ...
  try {
    ...
    if (Array.isArray(data.logs)) {
      data.logs.forEach((l: string) => onLog(l))
    }
...
function FilaPreview({
  ...
  const estado: EstadoPedido =
    resultado?.estado || (pedido.errores.length > 0 ? 'error' : 'pendiente')
    ...
    <>
      <tr
        ...
        className={`border-b border-dark-800 hover:bg-dark-800/50 cursor-pointer transition-colors ${
          tieneErroresValidacion ? 'bg-red-950/20' : ''
        }`}
      >
        ...
        <td className="px-4 py-3">
          ...
          <span className="text-dark-500 text-xs ml-1">
            artículo{pedido.renglones.length !== 1 ? 's' : ''}
          </span>
        </td>
        ...
        <td className="px-4 py-3">
          {tieneErroresValidacion ? (
            <span className="text-red-400 text-xs font-medium">
              ⚠ {pedido.errores.length} error{pedido.errores.length > 1 ? 'es' : ''}
            </span>
          ) : (
                    ...
                    {pedido.renglones.map((r, i) => (
                      <tr key={i} className="border-t border-dark-800">
                        ...
                        <td className="py-1 text-right text-yellow-400">
                          {r.bonifpct && r.bonifpct !== '0' && r.bonifpct !== ''
                            ? r.bonifpct + '%'
                            : '—'}
                        </td>
                        <td className="py-1 text-right text-dark-400 text-xs">
                          {r.motivo && MOTIVOS_VALIDOS[r.motivo] ? MOTIVOS_VALIDOS[r.motivo] : '—'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                ...
                {Object.keys(pedido.overrides).length > 0 && (
                  <div>
                    <p className="text-dark-400 text-xs font-medium uppercase tracking-wider mb-1">
                      Overrides de cabecera
                    </p>
                    {Object.entries(pedido.overrides).map(([k, v]) => (
                      <p key={k} className="text-dark-300 text-xs font-mono">
...
export default function CargaPedidos() {
  ...
  const ejecutarCarga = async () => {
    ...
    if (pedidosValidos.length === 0) {
      setError('No hay pedidos válidos para cargar')
      return
    }
    ...
    for (let i = 0; i < pedidosValidos.length; i++) {
      ...
      nuevosResultados.set(pedido.key, {
        key: pedido.key,
      ...
      const resultado = await procesarPedidoERP(pedido, msg => {
        logs.push(msg)
      })
  ...
  const progresoPct =
    pedidosValidos > 0 ? Math.round((progresoActual / pedidosValidos) * 100) : 0
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-dark-400 text-sm mt-1">
              Chess ERP — VAFOOD SRL · Carga masiva vía CSV
            </p>
          </div>
          ...
          {pedidos.length > 0 && (
            <div className="flex gap-2">
              {hayResultados && (
                <button
                  onClick={() => descargarCSV([...resultados.values()])}
        ...
        {pedidos.length === 0 && (
          ...
          >
            ...
            <div className="inline-flex gap-4 text-xs text-dark-500 flex-wrap justify-center">
              <span>✓ idcliente</span>
              <span>✓ idclialias</span>
              <span>✓ fecentre</span>
              <span>✓ codart</span>
              <span>✓ cant</span>
              <span>opcionales: idempresa / iddocumento</span>
            </div>
          ...
          <>
            <div className="card rounded-xl p-4 mb-4 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-dark-400 text-sm">
                  {rawRows.length} filas · {pedidos.length} pedidos agrupados · {totalArticulos} artículos totales
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              ...
              <div className={`card rounded-xl p-4 ${pedidosConError > 0 ? 'border-red-500/20' : ''}`}>
                <p className={`text-2xl font-bold ${pedidosConError > 0 ? 'text-red-400' : 'text-dark-500'}`}>
                  {pedidosConError}
                </p>
              ...
              <div className="card rounded-xl p-4 mb-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-dark-400 text-sm">
                    {progresoActual}/{pedidosValidos} ({progresoPct}%)
                  </span>
                </div>
            ...
            {hayResultados && !cargando && (
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="card rounded-xl p-3 border-green-500/20 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xl font-bold text-green-400">{resOk}</p>
                    <p className="text-dark-400 text-xs">Grabados OK</p>
                  </div>
                </div>
                ...
                <div className="card rounded-xl p-3 border-yellow-500/20 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xl font-bold text-yellow-400">{resAdvert}</p>
                    <p className="text-dark-400 text-xs">Advertencias</p>
                  </div>
                </div>
                ...
                <div className="card rounded-xl p-3 border-red-500/20 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xl font-bold text-red-400">{resError}</p>
                    <p className="text-dark-400 text-xs">Errores</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-dark-700">
                    <th className="text-left px-4 py-3 text-dark-400 text-xs font-medium uppercase tracking-wider w-10">
                      #
                    </th>
                    <th className="text-left px-4 py-3 text-dark-400 text-xs font-medium uppercase tracking-wider">
                      Cliente
                    </th>
                    <th className="text-left px-4 py-3 text-dark-400 text-xs font-medium uppercase tracking-wider">
                      Fecha entrega
                ...
                <tbody>
                  {pedidos.map((pedido, i) => (
                    <FilaPreview
                      key={pedido.key}
                      pedido={pedido}
                      resultado={resultados.get(pedido.key)}
                      index={i}
                    />
                  ))}
                </tbody>
